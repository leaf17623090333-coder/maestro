...
- Planless flow sections should be appended after all plan-based steps, separated by a horizontal rule -- keeps existing flow untouched (from review-no-plan.md)
- The SKILL.md description field should reflect all modes the skill supports (from review-no-plan.md)
- All Maestro commands use AskUserQuestion with consistent formatting for multi-choice selections (from work-plan-autoload.md)
- Plan format strings in design.md are single-line escaped strings embedded in Task() prompts -- hard for agents to edit reliably (from improve-design-workflow.md)
